...
const TheseEdit = () => {
    ...
    const { id } = useParams(); // Get the thesis ID from the URL
    ...
    useEffect(() => {
        ...
        const fetchThesis = async () => {
            try {
                ...
                setLieu(thesis.lieu);
                setDate(thesis.date);
                setSelectedAuthors(thesis.author.split(', '));
                setSelectedAuthorIds(thesis.id_user.split(','));
            } catch (error) {
                console.error('Erreur lors de la récupération de la thèse:', error);
                setError('Erreur lors de la récupération de la thèse');
                toast.error('Erreur lors de la récupération de la thèse');
            }
        };

        fetchMembers();
        if (id) {
            fetchThesis(); // Fetch thesis data if editing
        }
    }, [accessToken, id]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:8000/api/theses/${id}`, {
                title,
                doi,
            ...
            console.log('Thèse modifiée:', response.data);
            toast.success('Thèse modifiée avec succès');
            navigate('/dashboard/theses');
        } catch (error) {
            console.error('Erreur lors de la modification de la thèse:', error.response?.data || error.message);
            setError(`Erreur lors de la modification de la thèse: ${error.response?.data?.message || error.message}`);
            toast.error(`Erreur lors de la modification de la thèse: ${error.response?.data?.message || error.message}`);
        }
    };
